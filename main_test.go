...
`
	configFileWithNoSampleJobs = `servers = []
listen = "127.0.0.1:3000"
version = "0.1"
[auth]
  enabled = false
...
[sample]
  storage = "{\"jobs\":[],\"tubes\":[{\"name\":\"default\",\"keys\":[]},{\"name\":\"aurora_test\",\"keys\":[]}]}"
`
)
...
var (
	...
	urls = []string{
		...
		"/tube?server=127.0.0.1:11300&tube=default1",                                                                                          // Tube status with no exits tube
		...
		"/tube?server=127.0.0.1:11300&tube=default&action=loadSample&key=97ec882fd75855dfa1b4bd00d4a367d4&redirect=tube&action=manageSamples", // Load sample job by given key
...
func TestCurrentTubeJobsActionsRow(t *testing.T) {
	...
	if _, err := toml.Decode(configFileWithNoSampleJobs, &pubConf); err != nil {
		t.Log(err)
		return
	}
